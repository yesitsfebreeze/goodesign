...
Make these moves and most design tasks collapse to a fraction of their apparent size.
...
**Here:** the constraint list *is* `core/contract.md` move 0 and move 1.
...
**Here:** `craft/space.md`. Set the scale before you place anything.
...
**Here:** `craft/type.md` is decided second, immediately after space, and
...
scale.
**Here:** two faces is the ceiling in `craft/type.md`. Before adding a
...
**Here:** when a review turns up seven findings, ask which single
...
work.
**Here:** the antidote to describing ten options. Commit to the first instinct, make it, and judge the made thing — do not talk yourself around the
circle first. `core/beliefs.md`: make the two best options; never describe ten.
...
much.
**Here:** when the gate in `process/gate.md` will not pass, the first
...
**Here:** `craft/space.md` — empty may stay empty. This is the reason
...
**Here:** the general form of `core/beliefs.md` #9 and the direct rule
...
**Here:** make the ugly version in the real medium — real screen, real copy,
real data — before discussing it. A rendered thing at the wrong size
...
**Here:** the single largest efficiency lever available. Arrange to see each change the moment it is made before designing in
earnest. Iterating on a picture of the thing is slower than iterating on the
thing, every time.
...
**Here:** the design is not done when it is handed over; it is done when it
is in someone's hands and holds. A design that cannot be made as described was
...
**Here:** level 0 of `process/review.md`, and the reason it comes before
...
than lucky.
**Here:** `process/gate.md` exists to be walked, reported line by line, and
...
**Here:** you rarely have five users. You always have heuristic
...
taste.
**Here:** `flow/judgement.md`. Known, assumed, or taste — pick one for
every finding.
...
An animation short enough to go unnoticed reads as snappy; one long enough
to watch, on a frequent action, reads as frustrating.
**Here:** `craft/motion.md`. The budget is set by frequency before it is
set by taste.
...
**Here:** `craft/states.md` and `craft/motion.md`, for anything dragged.
...
- **Describing options instead of making them.** Ten described directions cost
  more than two made ones and settle nothing.
...
- **Iterating on a description rather than on the thing.**
